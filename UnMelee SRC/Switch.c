...
void LoadSwap(GOBJ *fighter_gobj, ArchiveInfo *wpind_archive)
{
	...
	Symbol_WpInd_scene_models *symbol_WpInd_scene_models = File_GetSymbol(wpind_archive, "Swap_scene_models");
	JOBJSet *jobjset_WpInd = symbol_WpInd_scene_models->WpInd_scene_models;
	// GOBJ *gobj_Tension = GObj_Create(3, 0, 0);
	GOBJ *gobj_WpInd = JOBJ_LoadSet(0, jobjset_WpInd, 0, 0, 0, 11, 1, SwapLogic);
...
//        TENSION GUAGE        //
/////////////////////////////////
void SwapLogic(GOBJ *gobj)//, GOBJ *gobj_Tension)
{
	
    JOBJ *jobj_Swap = gobj->hsd_object;
    JOBJ *jobj_CurrCharge; //Number group JOBJ
    JOBJ_GetChild(jobj_Swap, &jobj_CurrCharge, 0, -1); //Swapt 0, meter and shit
    ...
    SwapOnframe(Tension_data->fighter_gobj, gobj);
    ...
    if (Tension_data->fighter_gobj != 0 && (Pause_CheckStatus(1) != 2) && (Fighter_GetStocks(Tension_data->ply) > 0))
    {
        ...
        jobj_Swap->trans = *Match_GetPlayerHUDPos(fighterData->ply);
        jobj_Swap->trans.X = jobj_Swap->trans.X -4.6; //-5.04;
        jobj_Swap->trans.Y = jobj_Swap->trans.Y + 0.95;
		// jobj_Swap->trans.X = jobj_Swap->trans.X -0;
        // jobj_Swap->trans.Y = jobj_Swap->trans.Y - 0.75;
		// jobj_Swap->scale.Y = -1;
		// jobj_Swap->rot.Z = -1.57;
        ...
        JOBJ_ClearFlagsAll(jobj_Swap, JOBJ_HIDDEN);
        JOBJ_SetMtxDirtySub(jobj_Swap);
		...
		{
			Tension_data->Tension += 15;
		}

		if(fighterData->state_id < 23 & fighterData->state_id > 14)
		{
			Tension_data->Tension += 2;
		}
    ...
    {
        JOBJ_SetFlagsAll(jobj_Swap, JOBJ_HIDDEN);
    }
...
void SwapOnframe(GOBJ *fighter_gobj, GOBJ *tension_gobj)
{
						...
						{   
							...
							else
							{
								for (int i = 0; i < 4; i++)
								{
									...
									if (Fighter_GetGObj(i) != 0 && Fighter_GetGObj(i) != fighterData->ply)
									{
										...
										{
											ActionStateChange(0,1,0,Fighter_GetGObj(i),ASID_FALL,0,0);
										}
										
										if ((fighterData->state_id == ASID_ESCAPEAIR) || (!(fighterData->jump.jumps_used == 0 && victimData->jump.jumps_used == 0)))
										{
											ActionStateChange(0,1,0,fighter_gobj,ASID_FALL,0,0);
										}

										pos = victimData->phys.pos;
										victimData->phys.pos = fighterData->phys.pos;
										fighterData->phys.pos = pos;

										Effect_SpawnAsyncLookup(fighter_gobj, 233, 4, 0, 1, &zero, &zero);
										Effect_SpawnAsyncLookup(Fighter_GetGObj(i), 233, 4, 0, 1, &zero, &zero);
									}
								}		
