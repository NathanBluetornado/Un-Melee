...
void LoadJoin(GOBJ *fighter_gobj, ArchiveInfo *wpind_archive)
{
	...
	Symbol_WpInd_scene_models *symbol_WpInd_scene_models = File_GetSymbol(wpind_archive, "Join_scene_models");
	JOBJSet *jobjset_WpInd = symbol_WpInd_scene_models->WpInd_scene_models;
	// GOBJ *gobj_Tension = GObj_Create(3, 0, 0);
	GOBJ *gobj_WpInd = JOBJ_LoadSet(0, jobjset_WpInd, 0, 0, 0, 11, 1, JoinLogic);
...
//        TENSION GUAGE        //
/////////////////////////////////
void JoinLogic(GOBJ *gobj)//, GOBJ *gobj_Tension)
{
	
    JOBJ *jobj_Join = gobj->hsd_object;
    JOBJ *jobj_CurrCharge; //Number group JOBJ
    JOBJ_GetChild(jobj_Join, &jobj_CurrCharge, 0, -1); //joint 0, meter and shit
    ...
    JoinOnframe(Tension_data->fighter_gobj, gobj);
    ...
    if (Tension_data->fighter_gobj != 0 && (Pause_CheckStatus(1) != 2) && (Fighter_GetStocks(Tension_data->ply) > 0))
    {
        ...
        jobj_Join->trans = *Match_GetPlayerHUDPos(fighterData->ply);
        jobj_Join->trans.X = jobj_Join->trans.X -4.6; //-5.04;
        jobj_Join->trans.Y = jobj_Join->trans.Y + 0.95;
		// jobj_Join->trans.X = jobj_Join->trans.X -0;
        // jobj_Join->trans.Y = jobj_Join->trans.Y - 0.75;
		// jobj_Join->scale.Y = -1;
		// jobj_Join->rot.Z = -1.57;
        ...
        JOBJ_ClearFlagsAll(jobj_Join, JOBJ_HIDDEN);
        JOBJ_SetMtxDirtySub(jobj_Join);
    ...
    {
        JOBJ_SetFlagsAll(jobj_Join, JOBJ_HIDDEN);
    }
							...
							else
							{
								for (int i = 0; i < 4; i++)
								{
									...
									if (GetEnemyPosition(fighterData, i, &pos) != 0)
									{
										...
										fighterData->phys.pos = victimData->phys.pos;
										// update ecb pos
										fighterData->coll_data.topN_Curr = fighterData->phys.pos;
										fighterData->coll_data.topN_Prev = fighterData->coll_data.topN_Curr;
										// ActionStateChange(fighterData->state.frame, fighterData->state.rate, fighterData->state.blend, fighter_gobj, fighterData->state_id, 0, 0);
										// Fighter_SetPosition(fighterData->ply, fighterData->flags.ms, &victimData->phys.pos);
									}
								}		
								...
								Tension_data->Tension -= 2000;
								Effect_SpawnAsyncLookup(fighter_gobj, 30, 4, 0, 1, &zero, &zero);
							}
						}
					}
				}
