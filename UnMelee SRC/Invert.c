...
void LoadMeteor(GOBJ *fighter_gobj)
{
	...
	Symbol_WpInd_scene_models *symbol_WpInd_scene_models = File_GetSymbol(wpind_archive, "Meteor_scene_models");
	JOBJSet *jobjset_WpInd = symbol_WpInd_scene_models->WpInd_scene_models;
	// GOBJ *gobj_Tension = GObj_Create(3, 0, 0);
	GOBJ *gobj_WpInd = JOBJ_LoadSet(0, jobjset_WpInd, 0, 0, 0, 11, 1, MeteorLogic);
...
//        TENSION GUAGE        //
/////////////////////////////////
void MeteorLogic(GOBJ *gobj)//, GOBJ *gobj_Tension)
{
	
    JOBJ *jobj_Meteor = gobj->hsd_object;
    JOBJ *jobj_CurrCharge; //Number group JOBJ
    JOBJ_GetChild(jobj_Meteor, &jobj_CurrCharge, 0, -1); //Meteort 0, meter and shit
    ...
    MeteorOnframe(Tension_data->fighter_gobj, gobj);
    ...
    if (Tension_data->fighter_gobj != 0 && (Pause_CheckStatus(1) != 2) && (Fighter_GetStocks(Tension_data->ply) > 0))
    {
        ...
        jobj_Meteor->trans = *Match_GetPlayerHUDPos(fighterData->ply);
        jobj_Meteor->trans.X = jobj_Meteor->trans.X -4.6; //-5.04;
        jobj_Meteor->trans.Y = jobj_Meteor->trans.Y + 0.95;
		// jobj_Meteor->trans.X = jobj_Meteor->trans.X -0;
        // jobj_Meteor->trans.Y = jobj_Meteor->trans.Y - 0.75;
		// jobj_Meteor->scale.Y = -1;
		// jobj_Meteor->rot.Z = -1.57;
        ...
        JOBJ_ClearFlagsAll(jobj_Meteor, JOBJ_HIDDEN);
        JOBJ_SetMtxDirtySub(jobj_Meteor);
    ...
    {
        JOBJ_SetFlagsAll(jobj_Meteor, JOBJ_HIDDEN);
    }
...
void MeteorOnframe(GOBJ *fighter_gobj, GOBJ *tension_gobj)
{
								...
								for (int i = 0; i < 4; i++)
								{
									...
									if (GetEnemyPosition(fighterData, i, &pos) != 0)
									{
										...
										if (&victimData->phys.kb_vel != &zero)
										{
											...
											// if (victimData->phys.kb_vel.Y > victimData->phys.kb_vel.X)
											// {
											// 	victimData->phys.kb_vel.Y = -victimData->phys.kb_vel.Y;
											// }
											// else
											// {
												victimData->phys.kb_vel.X = -victimData->phys.kb_vel.X;
											// }


											Effect_SpawnAsyncLookup(Fighter_GetGObj(i), 67, 4, 0, 1, &zero, &zero);									
