...
void LoadPull(GOBJ *fighter_gobj, ArchiveInfo *wpind_archive)
{
	...
	Symbol_WpInd_scene_models *symbol_WpInd_scene_models = File_GetSymbol(wpind_archive, "Pull_scene_models");
	JOBJSet *jobjset_WpInd = symbol_WpInd_scene_models->WpInd_scene_models;
	// GOBJ *gobj_Tension = GObj_Create(3, 0, 0);
	GOBJ *gobj_WpInd = JOBJ_LoadSet(0, jobjset_WpInd, 0, 0, 0, 11, 1, PullLogic);
...
//        TENSION GUAGE        //
/////////////////////////////////
void PullLogic(GOBJ *gobj)//, GOBJ *gobj_Tension)
{
	
    JOBJ *jobj_Pull = gobj->hsd_object;
    JOBJ *jobj_CurrCharge; //Number group JOBJ
    JOBJ_GetChild(jobj_Pull, &jobj_CurrCharge, 0, -1); //joint 0, meter and shit
    ...
    PullOnframe(Tension_data->fighter_gobj, gobj);
    ...
    if (Tension_data->fighter_gobj != 0 && (Pause_CheckStatus(1) != 2) && (Fighter_GetStocks(Tension_data->ply) > 0))
    {
        ...
        jobj_Pull->trans = *Match_GetPlayerHUDPos(fighterData->ply);
        jobj_Pull->trans.X = jobj_Pull->trans.X -4.6; //-5.04;
        jobj_Pull->trans.Y = jobj_Pull->trans.Y + 0.95;
		// jobj_Pull->trans.X = jobj_Pull->trans.X -0;
        // jobj_Pull->trans.Y = jobj_Pull->trans.Y - 0.75;
		// jobj_Pull->scale.Y = -1;
		// jobj_Pull->rot.Z = -1.57;
        ...
        JOBJ_ClearFlagsAll(jobj_Pull, JOBJ_HIDDEN);
        JOBJ_SetMtxDirtySub(jobj_Pull);
		...
		if (fighterData->hitbox->active != 0)
		{
    ...
    {
        JOBJ_SetFlagsAll(jobj_Pull, JOBJ_HIDDEN);
    }
							...
							else
							{
								for (int i = 0; i < 4; i++)
								{
									...
									if (GetEnemyPosition(fighterData, i, &pos) != 0)
									{
										...
										if((fighterData->state_id == ASID_ESCAPEAIR) || (victimData->jump.jumps_used == 0))
										{
											ActionStateChange(0,1,0,Fighter_GetGObj(i),ASID_FALL,0,0);
										}
										victimData->phys.pos = fighterData->phys.pos;
										// update ecb pos
										// victimData->coll_data.topN_Curr = victimData->phys.pos;
										// victimData->coll_data.topN_Prev = victimData->coll_data.topN_Curr;
									}
								}		
